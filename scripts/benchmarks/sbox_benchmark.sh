...
run_bench() {
    ...
    # 1. Extract Logic Depth (LD) from the specific module path
    ltp_full=$(grep "Longest topological path in ${TOP_MODULE}" $LOG | tail -n 1)
    depth=$(echo "$ltp_full" | sed 's/.*length=\([0-9]*\).*/\1/')
    depth="${depth:-0}"
    
    echo "TIMING ANALYSIS:"
    echo "$ltp_full"

    # 2. Extract ONLY the FINAL Resource Stats (Post-ABC)
    # Uses tac to find the last occurrence of the module header to ensure final results
    raw_stats=$(tac $LOG | awk '/^=== '"$TOP_MODULE"' ===/ {exit} {print}' | tac | awk '/cells/ || /^[[:space:]]+[0-9]+[[:space:]]+\$_/')
